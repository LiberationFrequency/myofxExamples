...
void ofApp::setup(){	 
	...
	soundStream.setup(this, 2, 2, 44100, bufferSize, 4);
        ...
        audioBuffer[0] = &left[0];
        audioBuffer[1] = &right[0];
...
//     reverb.init(sampleRate); // initializing the Faust module
       reverb.init(44100); // initializing the Faust module
       reverb.buildUserInterface(&reverbControl); // linking the Faust module to the controler
    
//     // Allocate our member audio buffer
//     audioBuffer = (float**)malloc(sizeof(float*)*nInputChans);
//     for(int i=0; i<nInputChans; i++){
//        audioBuffer[i] = (float*)malloc(sizeof(float)*bufferSize*2);
//     }
       ...
       for(int i=0; i<reverbControl.getParamsCount(); i++){
          std::cout << reverbControl.getParamAddress(i) << "\n";
    }
...
void ofApp::draw(){
	...
	ofSetColor(225);
	ofDrawBitmapString("AUDIO INPUT/FX/OUTPUT EXAMPLE", 32, 32);
